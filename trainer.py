...
from utils.log import save_model,data_reshaper
from utils.dataloader import DataLoader
from utils.util import masked_mae,masked_rmse,masked_mape


class trainer():
    def __init__(self, scaler, model, **optim_args):
        self.model  = model         # init modela
    ...
    def test(model, save_path_resume, device, dataloader, scaler, model_name, save=True, **kwargs):
        ...
        for i in range(12):
            pred    = yhat[:,:,i]
            real    = realy[:,:,i]
            # if model_name == 'METR-LA':
                ##METR-LA has too many missing values.
